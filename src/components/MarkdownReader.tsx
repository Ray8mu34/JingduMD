...
function MarkdownPre({ children }: { children?: ReactNode }) {
  ...
  if (className.includes("math-") || className.includes("language-mermaid")) return <pre>{children}</pre>;
...
function LazyMath({ source, display, loadMargin }: { source: string; display: boolean; loadMargin: string }) {
  ...
  return <span ref={ref} className={display ? "lazy-math math-display" : "lazy-math math-inline"} {...(html ? { dangerouslySetInnerHTML: { __html: html } } : {})}>{html ? undefined : source}</span>;
}
  ...
  useEffect(() => {
    ...
    import("mermaid").then(async ({ default: mermaid }) => {
      ...
      try {
        ...
        if (active) setSvg(sanitizeMermaidSvg(result.svg));
      } catch (reason) { if (active) setError(String(reason)); }
    });
    return () => { active = false; };
  }, [source, visible, night]);
  return <div ref={ref} className="mermaid-block">{svg ? <div dangerouslySetInnerHTML={{ __html: svg }} /> : error ? <pre className="diagram-error">{error}</pre> : "图表加载中…"}</div>;
}
