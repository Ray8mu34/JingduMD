...
type Props = { value: ReaderPreferences; sampleStyle?: CSSProperties; onChange: (value: ReaderPreferences) => void; onClose: (reason?: "button" | "tab") => void; onLegacyEdit: () => void };
...
export default function SimpleReadingSettings({ value, sampleStyle, onChange, onClose, onLegacyEdit }: Props) {
  ...
  const savePersonal = () => {
    ...
    change({ ...value, personalTypographies: [...value.personalTypographies, { id: globalThis.crypto?.randomUUID?.() ?? String(Date.now()), name: label, profile: value.typographyProfile, overrides: {
      ...
      chineseHeadingFont: value.typographyOverrides[value.typographyProfile]?.chineseHeadingFont === undefined && !effective.chineseHeadingFont ? undefined : effective.chineseHeadingFont,
      latinHeadingFont: value.typographyOverrides[value.typographyProfile]?.latinHeadingFont === undefined && !effective.latinHeadingFont ? undefined : effective.latinHeadingFont,
  ...
  return <aside ref={panel} className={`settings-sheet simple-settings page-${page}`} role="dialog" aria-label={title} onBlur={(event) => { if (event.relatedTarget && !event.currentTarget.contains(event.relatedTarget as Node)) onClose("tab"); }} onKeyDown={(event) => { if (event.key === "Escape" && page !== "quick") { event.preventDefault(); event.stopPropagation(); setPage("quick"); } }}>
    ...
    <div className="simple-settings-body">
      {(page === "quick" || page === "paragraph") && <div className="settings-preview" aria-label="实时排版预览"><small>实时预览</small><div className="settings-preview-content" style={sampleStyle}><strong>阅读，留一点从容 · Notes 2026</strong><p>文字之间有呼吸，段落之间有停顿。让每一行 Chinese 与 English 都清楚自然。</p></div></div>}
      {page === "quick" && <>
        ...
        <p className="setting-help">阅读：书页式标题与舒展行距。研读：清晰标题与较紧凑的行距。两套排版分别保留你的调整。</p>
        ...
        <p className="setting-help">栏宽上限约 {Math.round(effective.contentWidth / effective.fontSize)} 个汉字／行；窄窗口会自动适配。</p>
      ...
      {page === "paragraph" && <>
        ...
        <p className="setting-help">行距是字号的倍数；1 em 相当于一个正文字号。当前栏宽上限约 {Math.round(effective.contentWidth / effective.fontSize)} 字／行。</p>
        {value.styleMode === "canonical" && <><button className="text-action" onClick={() => change({ ...value, typographyOverrides: { ...value.typographyOverrides, [value.typographyProfile]: {} } })}>恢复当前排版默认值</button><p className="setting-help">只恢复当前“{value.typographyProfile === "reading" ? "阅读" : "研读"}”的排版调整，保留字号与外观。</p></>}
        {touched.current.size > 0 && <button className="text-action" onClick={undo}>撤销本次调整</button>}
